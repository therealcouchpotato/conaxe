...
import static io.conaxe.ByteUtil.bytesRangeToString;


// RXTX Event driven implementation of the emu

public class EventDrivenConaxCard implements SerialPortEventListener {
    ...
    private int resetSignal;
    private final boolean resetIdleState;
    private final String resetSignalName;

    private CardServerGateway cardServerGateway;
    private int failReadCount;
    ...
    public EventDrivenConaxCard(ConsoleOutput console, ConaxTransport transport, ConaxCardInfo cardInfo, ConaxNanoAssembler assembler, ConaxNanoDissector dissector,
                     CardServerGateway cardServerGateway, String resetSignal, boolean invertedResetSignal) {
        this.console = console;
        this.transport = transport;
        this.cardInfo = cardInfo;
        this.cardServerGateway = cardServerGateway;
        answerQueue = new ByteArrayQueue(254);
        this.resetSignalName = resetSignal;

        resetIdleState = (invertedResetSignal == false);
    ...
    private void initEventListener() {
        try {
            ...
            if (resetSignalName.toLowerCase().equals("cts")) {
                this.resetSignal = SerialPortEvent.CTS;
                transport.getSerialPort().notifyOnCTS(true);
            }
            else if (resetSignalName.toLowerCase().equals("dsr")) {
                this.resetSignal = SerialPortEvent.DSR;
                transport.getSerialPort().notifyOnDSR(true);
            }
            else if (resetSignalName.toLowerCase().equals("dcd")) {
                this.resetSignal = SerialPortEvent.CD;
                transport.getSerialPort().notifyOnCarrierDetect(true);
            }
            else if (resetSignalName.toLowerCase().equals("ri")) {
                this.resetSignal = SerialPortEvent.RI;
                transport.getSerialPort().notifyOnRingIndicator(true);
            } else {
                console.println("Missing resetSignal definition, defaulting to CTS");
                this.resetSignal = SerialPortEvent.CTS;
                transport.getSerialPort().notifyOnCTS(true);
            }

            transport.getSerialPort().notifyOnDataAvailable(true);
            transport.getSerialPort().addEventListener(this);

        } catch (TooManyListenersException e) {
            console.println("Could not register eventlistener with rxtx");
        }
    }
    ...
    public void serialEvent(SerialPortEvent ev) {
        ...
        switch (ev.getEventType()) {
            case SerialPortEvent.DATA_AVAILABLE:
                ...
                try {
                    ...
                    transport.reset();
                    console.println("[ConaxCard] read error");
                }
                ...
                if (failReadCount >100) {
                   // transport.reinitialize();
                   // initEventListener();
                    failReadCount = 0;
                    transport.reset();
                    console.println("..");
            ...
            case SerialPortEvent.CTS:
                if (resetSignal == SerialPortEvent.CTS) {
                    isReset = true;
                }
                break;
            case SerialPortEvent.DSR:
                if (resetSignal == SerialPortEvent.DSR) {
                    isReset = true;
                }
                break;
            case SerialPortEvent.CD:
                if (resetSignal == SerialPortEvent.CD) {
                    isReset = true;
                }
                break;
            case SerialPortEvent.RI:
                if (resetSignal == SerialPortEvent.RI) {
                    isReset = true;
                }
                break;
        ...
        if (isReset) {
            if (ev.getOldValue() == resetIdleState && ev.getNewValue() == !resetIdleState) {
                try {
                    answerQueue.clear();
                    try {Thread.sleep(10);} catch (Throwable t){}
                    transport.reset();
    ...
    private void handleCommand(ConaxCommand command)
    {
        ...
        switch (command.getInstruction())
        {
            ...
            case CC_CARD_IDENTIFY:
                ...
                byte[] camSerial = Arrays.copyOfRange(camIdentityNanoData, 0, 7);  // not actually a camSerial, don't know what
            ...
            case CC_ECM:
                ...
                {
                    console.println("[ConaxCard] ECM format unknown. Maybe box is paired?");
                    return;
                }
			...
			// TODO: actually keep and use pin setting
